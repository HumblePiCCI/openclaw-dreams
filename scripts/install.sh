...
openclaw cron add \
  ...
  --tz "$TZ_NAME" \
  --session isolated \
  --message "Run memory/dreams/PROTOCOL.md end-to-end. Write outputs to memory/dreams/log/YYYY-MM.md and overwrite memory/dreams/DREAMS.md using the order-of-operations layout. Include mixing mode + mixing map + imagery prompts. Use web_search + web_fetch. Do not message any humans."
